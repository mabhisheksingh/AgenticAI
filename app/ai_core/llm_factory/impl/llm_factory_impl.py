    ...
    def load_llm(self, model_name: str, temperature: float | None = None, with_tools: bool = True):
        ...
        try:
            if provider == "openai":
                ...
                llm = ChatOpenAI(
                    model=model_name, temperature=temp, api_key=SecretStr(api_key), timeout=timeout
                )
                ...
                llm = ChatGoogleGenerativeAI(
                    model=model_name, api_key=SecretStr(api_key), temperature=temp, timeout=timeout
                )
                ...
                llm = ChatGroq(model=model_name, api_key=SecretStr(api_key), timeout=timeout)
                ...
                llm = HuggingFacePipeline(
                    pipeline=pipe, model_kwargs={"temperature": temp, "max_length": 512}
                )
            else:
